#!/usr/bin/env python3
"""job-queue.py — routes tasks from AUTONOMOUS.md to OpenClaw agent sessions.
...
def get_pending_tasks() -> list[dict]:
    """Parse AUTONOMOUS.md for PENDING tasks.

    Returns list of {task_id, agent, title, body}.
    """
    if not AUTONOMOUS_FILE.exists():
        return []

    content = AUTONOMOUS_FILE.read_text(encoding="utf-8")
    tasks = []

    # Format: | TASK_ID | task-name | agent | status | ... |
    import re
    rows = re.findall(r'\| (AUTO-\d+) \| ([^|]+) \| ([^|]+) \|.*?\| (PENDING|IN_PROGRESS)', content)

    for task_id, task_name, agent_raw, status in rows:
        agent = agent_raw.strip().lower()
        if agent in VALID_AGENTS and status == "PENDING":
            tasks.append({
