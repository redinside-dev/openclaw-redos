...
if (import.meta.url === `file://${process.argv[1]}`) {
  // Support both "--agent=eng" and "eng" formats
  const rawArg = process.argv[2] || 'worker';
  const agentId = rawArg.startsWith('--agent=') ? rawArg.slice('--agent='.length) : rawArg;
  const agentName = process.argv[3] || `Autonomous Worker (${agentId})`;
